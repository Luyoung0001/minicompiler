...
/* 创建函数定义节点 */
ASTNode *ast_new_func_def(const char *ret_type, const char *name, ASTList *params, ASTNode *body, int line, int col) {
        ...
        case AST_FUNC_DEF:
            ...
            if (node->data.func_def.params) {
                for (int i = 0; i < node->data.func_def.params->count; i++) {
                    ASTNode *param = node->data.func_def.params->nodes[i];
                    if (i > 0) printf(", ");
                    printf("%s %s", param->data.var_decl.var_type, param->data.var_decl.var_name);
                }
            }
            printf(")\n");
