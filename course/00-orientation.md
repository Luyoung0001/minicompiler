...
> **预计时间**：45 ~ 90 分钟  
> **本章目标**：确认 `gcc`、RISC-V 工具链、QEMU 和课程 practice 都已经处于可继续推进的状态

正式课程会从下一章开始进入编译器本身，但在那之前，你得先把起跑线摆正。这里的“准备”不是为了多看一页导言，而是为了确保你后面每次读 chapter、做 lab、看到测试输出时，都知道自己到底在看什么、改什么、以及哪里出了问题。

做完这一章，你还不需要已经懂词法分析、AST 或 RISC-V 调用约定。你只需要进入一个很明确的工作状态：机器上的工具是通的，参考实现和课程 practice 的关系是清楚的，第一次 smoke test 已经跑过，接下来可以直接进入 `Chapter 1`。

## 0.1 这门课里有三套东西，它们分别干什么

先把三个角色分清楚，不然后面很容易改错地方。

1. 仓库根目录的 `step_1/` 到 `step_5/`：这是**参考实现**。它们展示了项目每个阶段的完成状态。
2. `course/`：这是**讲义**。你在这里读课程内容、看章节说明、查附录。
3. `course/practice/`：这是**学员实践区**。后面的每个 lab 都在这里完成，不直接改参考实现。

也就是说，这门课不是让你一上来钻进 `step_5/codegen.c` 直接改项目源码，而是让你先读 chapter，再去对应 lab 的骨架里补这章该写的代码。参考实现负责告诉你“成熟版本长什么样”，practice 负责给你一个真正能动手的练习边界。
...
4. 你已经在 `course/practice/labs/lab01-step1/` 跑过第一次 smoke test
...
- `step_1` 到 `step_5` 是项目逐步完成的参考版本
- `course/chapters/` 是主线讲义
- `course/practice/labs/` 是你真正下手写代码的地方

后面每章都会把这张粗地图收紧到当前这一章相关的 2~4 个文件上。

## 0.5 运行第一次 practice smoke test
...
| `No such file or directory` | 当前目录是不是 `course/practice/labs/...` |
...
1. [Practice 与实验说明](practice/README.md)
2. [Chapter 1 — 编译器最小驱动与文件 I/O](chapters/ch01-step1-driver.md)
